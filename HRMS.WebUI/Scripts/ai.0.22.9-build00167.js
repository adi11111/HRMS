                    ...
                    DataSanitizer.sanitizeProperties = function (properties) {
                        if (properties) {
                            ...
                            for (var prop in properties) {
                                ...
                                prop = DataSanitizer.sanitizeKeyAndAddUniqueness(prop, tHRMSProps);
                                tHRMSProps[prop] = value;
                            }
                            properties = tHRMSProps;
                        }
                        return properties;
                    };
                    DataSanitizer.sanitizeMeasurements = function (measurements) {
                        if (measurements) {
                            var tHRMSMeasurements = {};
                            for (var measure in measurements) {
                                var value = measurements[measure];
                                measure = DataSanitizer.sanitizeKeyAndAddUniqueness(measure, tHRMSMeasurements);
                                tHRMSMeasurements[measure] = value;
                            }
                            measurements = tHRMSMeasurements;
                        }
                        return measurements;
                    };
        ...
        var AppInsights = (function () {
            ...
            AppInsights.prototype.trackAjax = function (id, absoluteUrl, pathName, totalTime, success, resultCode) {
                if (this.config.maxAjaxCallsPerView === -1 ||
                    this._trackAjaxAttHRMSts < this.config.maxAjaxCallsPerView) {
                ...
                else if (this._trackAjaxAttHRMSts === this.config.maxAjaxCallsPerView) {
