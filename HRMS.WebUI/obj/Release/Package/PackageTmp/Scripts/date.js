	...
	var df = function (n) {
		return function () {
			...
			if (this._nth !== null) {
				...
				var ntHRMS = this._nth;
				this._nth = null;
				var tHRMS = this.clone().moveToLastDayOfMonth();
				this.moveToNthOccurrence(n, ntHRMS);
				if (this > tHRMS) {
					throw new RangeError($D.getDayName(n) + " does not occur " + ntHRMS + " times in the month of " + $D.getMonthName(tHRMS.getMonth()) + " " + tHRMS.getFullYear() + ".");
				}
				return this;
			}
...
(function () {
	...
	$D.Translator = {
		...
		finish: function (x) {
			var today, expression, orient, tHRMS;
			...
			if (this.weekday && this.unit !== "week" && !this.day && !this.days) {
				tHRMS = Date[this.weekday]();
				this.day = tHRMS.getDate();
				if (tHRMS.getMonth() !== today.getMonth()) {
					this.month = tHRMS.getMonth();
				}
			}
...
(function () {
	...
	var parseUtils = {
		...
		nativeFallback: function(s) {
			var t;
			try {
				// ok we haven't parsed it, last ditch attHRMSt with the built-in parser.
				t = Date._parse(s);
